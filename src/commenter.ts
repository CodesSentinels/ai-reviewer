/**
 * commenter.ts - GitHub PR 评论管理器（第二大源文件）
 *
 * 负责所有与 GitHub PR 评论相关的操作：
 *
 * 【评论管理】
 * - comment(): 创建或替换 PR 评论（通过 HTML 隐藏标签匹配）
 * - create() / replace(): 底层创建/更新评论方法
 * - findCommentWithTag(): 通过 HTML 标签查找已有评论
 * - listComments() / listReviewComments(): 分页获取评论（带缓存）
 *
 * 【审查评论】
 * - bufferReviewComment(): 缓冲审查评论（批量提交）
 * - submitReview(): 批量提交 GitHub Review（含回退到逐条评论的容错）
 * - deletePendingReview(): 清理未提交的待处理审查
 * - reviewCommentReply(): 回复审查评论
 *
 * 【评论链处理】
 * - getCommentChain(): 获取完整对话链
 * - composeCommentChain(): 组装对话链文本
 * - getCommentChainsWithinRange(): 获取指定行范围内的对话链
 *
 * 【状态追踪】
 * - getReviewedCommitIds() / addReviewedCommitId(): 追踪已审查的 commit
 * - addInProgressStatus() / removeInProgressStatus(): 审查进行中状态管理
 *
 * 【内容解析】
 * - getRawSummary() / getShortSummary(): 从评论中提取摘要
 * - getDescription() / getReleaseNotes(): 解析 PR 描述和发布说明
 * - updateDescription(): 将发布说明写入 PR 描述
 *
 * HTML 隐藏标签体系：
 * 使用 <!-- ... --> HTML 注释作为标记，在评论中嵌入元数据（摘要、commit ID 等）
 * 对用户不可见，但程序可通过标签定位和解析。
 */
...
/** 原始摘要的隐藏存储标签（存储在 HTML 注释中，用户不可见） */
...
/** 已审查 commit ID 列表的起止标签（用于增量审查） */
export const COMMIT_ID_START_TAG = '<!-- commit_ids_reviewed_start -->'
export const COMMIT_ID_END_TAG = '<!-- commit_ids_reviewed_end -->'

/**
 * Commenter 类 - GitHub PR 评论的统一管理器
 *
 * 内置缓存机制（issueCommentsCache / reviewCommentsCache）避免重复 API 调用。
 * 审查评论使用 buffer 模式：先缓冲到 reviewCommentsBuffer，最后批量提交。
 */
