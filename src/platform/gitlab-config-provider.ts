 ...
 */
const SECURITY_FORCED_FALSE = [
  'AI_REVIEWER_ENABLE_SHELL',
  ...
  getOptions(): Options {
    ...
    for (const key of SECURITY_FORCED_FALSE) {
      ...
      if (
        ...
        this.securityOverrides.push(
          `${key}="${raw}" overridden to false (CFG-002 security policy)`
        )
      }
    }
    ...
    this.cachedOptions = new Options(
      ...
      envBool('AI_REVIEWER_DISABLE_RELEASE_NOTES') ??
        CONFIG_DEFAULTS.disableReleaseNotes,
      envIntStr('AI_REVIEWER_MAX_FILES', CONFIG_DEFAULTS.maxFiles),
      envBool('AI_REVIEWER_REVIEW_SIMPLE_CHANGES') ??
        CONFIG_DEFAULTS.reviewSimpleChanges,
      envBool('AI_REVIEWER_REVIEW_COMMENT_LGTM') ??
        CONFIG_DEFAULTS.reviewCommentLGTM,
      envMultiline('AI_REVIEWER_PATH_FILTERS') ?? [
        ...CONFIG_DEFAULTS.pathFilters
      ],
      envStr('AI_REVIEWER_SYSTEM_MESSAGE') ?? CONFIG_DEFAULTS.systemMessage,
      envStr('AI_REVIEWER_OPENAI_LIGHT_MODEL') ??
        CONFIG_DEFAULTS.openaiLightModel,
      envStr('AI_REVIEWER_OPENAI_HEAVY_MODEL') ??
        CONFIG_DEFAULTS.openaiHeavyModel,
      envFloatStr(
        'AI_REVIEWER_OPENAI_MODEL_TEMPERATURE',
        CONFIG_DEFAULTS.openaiModelTemperature
      ),
      envIntStr('AI_REVIEWER_OPENAI_RETRIES', CONFIG_DEFAULTS.openaiRetries),
      envIntStr(
        'AI_REVIEWER_OPENAI_TIMEOUT_MS',
        CONFIG_DEFAULTS.openaiTimeoutMS
      ),
      envIntStr(
        'AI_REVIEWER_OPENAI_CONCURRENCY_LIMIT',
        CONFIG_DEFAULTS.openaiConcurrencyLimit
      ),
      envIntStr(
      ...
      envBool('AI_REVIEWER_ENABLE_DEPENDENCY_ANALYSIS') ??
        CONFIG_DEFAULTS.enableDependencyAnalysis,
      envIntStr(
        'AI_REVIEWER_MAX_DEPENDENCY_FILES',
        CONFIG_DEFAULTS.maxDependencyFiles
      ),
      envBool('AI_REVIEWER_ENABLE_WEB_SEARCH') ??
        CONFIG_DEFAULTS.enableWebSearch,
      ...
      envStr('AI_REVIEWER_COMMAND_ACK_REACTION') ??
        CONFIG_DEFAULTS.commandAckReaction,
      envIntStr(
        'AI_REVIEWER_MAX_REVIEW_COMMENTS',
        CONFIG_DEFAULTS.maxReviewComments
      ),
      envIntStr(
  ...
  getPromptConfig(): PromptConfig {
    ...
    this.cachedPromptConfig = {
      ...
      summarizeReleaseNotes:
        envStr('AI_REVIEWER_SUMMARIZE_RELEASE_NOTES') ??
