/**
 * review-comment.ts - PR 评论回复处理器
 *
 * 处理 pull_request_review_comment 事件，即用户在审查评论中与 Bot 的对话。
 *
 * 触发条件（必须满足以下之一）：
 * - 评论链中已包含 Bot 的回复（COMMENT_TAG / COMMENT_REPLY_TAG）
 * - 用户评论中 @ai-reviewer 明确召唤 Bot
 *
 * 处理流程：
 * 1. 验证事件类型和 payload 完整性
 * 2. 排除 Bot 自身的评论（防止自我回复循环）
 * 3. 获取评论对话链（commentChain）
 * 4. 获取文件 diff 和 PR 摘要作为上下文
 * 5. Token 预算管理：按优先级逐步填充上下文（diff > fileDiff > shortSummary）
 * 6. 调用 heavyBot 生成回复
 * 7. 通过 Commenter 发布回复
 */
...
/** Bot 召唤关键词 - 用户通过此标记主动与 Bot 对话 */
const ASK_BOT = '@ai-reviewer'

/**
 * handleReviewComment - 处理审查评论回复事件
 *
 * @param heavyBot - 重量模型 Bot（用于生成高质量回复）
 * @param options - 全局配置
 * @param prompts - 提示词模板
 */
